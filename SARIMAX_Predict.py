...
# 데이터 로드
df = pd.read_csv('feat_imp_news_df.csv', parse_dates=['date'])
df.set_index('date', inplace=True)

# 학습/테스트 분리
train_df = df.loc[:'2024-09'].copy()
test_df = df.loc['2024-10':'2025-03'].copy()

# 타겟 변수와 외생 변수 분리
y_train = train_df['export_restored']
X_train = train_df.drop(['export_restored', 'export'], axis=1)
y_test = test_df['export_restored']
X_test = test_df.drop(['export_restored', 'export'], axis=1)
...
# 예측 결과 출력
print("\n=== 2024년 10월-2025년 3월 수출액 예측 결과 (SARIMAX) ===")
...
for idx, row in results_df.iterrows():
    plt.annotate(f'{row["실제 수출액"]:,.0f}', xy=(row['date'], row["실제 수출액"]),
                 xytext=(0, 24), textcoords='offset points', ha='center', va='top', fontsize=24, fontweight='bold')
    plt.annotate(f'{row["예측 수출액"]:,.0f}', xy=(row['date'], row["예측 수출액"]),
                 xytext=(0, -18), textcoords='offset points', ha='center', va='bottom', fontsize=24, fontweight='bold')
